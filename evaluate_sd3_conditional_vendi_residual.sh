...
export RESIDUAL_ORIGIN_LAYER="${RESIDUAL_ORIGIN_LAYER:-1}"
export RESIDUAL_TARGET_LAYERS="${RESIDUAL_TARGET_LAYERS:-$(seq -s ' ' 2 21)}"
export RESIDUAL_WEIGHTS="${RESIDUAL_WEIGHTS:-$(printf '0.025 %.0s' $(seq 2 21))}"
...
export TIMESTEP_RESIDUAL_WEIGHT_EXP_ALPHA="${TIMESTEP_RESIDUAL_WEIGHT_EXP_ALPHA:-0.0}"
export TIMESTEP_RESIDUAL_WEIGHT_POWER="${TIMESTEP_RESIDUAL_WEIGHT_POWER:-1.0}"
export TIMESTEP_STAGE="${TIMESTEP_STAGE:-0}"
export RESIDUAL_PROCRUSTES_PATH="${RESIDUAL_PROCRUSTES_PATH:-/inspire/hdd/project/chineseculture/public/yuxuan/Training-free-Residual-SD3/logs/procrustes_rotations/procrustes_rotations_coco5k_ln_t1-o1.pt}"

BASE_OUTPUT_PREFIX="${BASE_OUTPUT_PREFIX:-$SCRIPT_DIR/logs/conditional_vendi_coco5k_residual}"
...
for run_idx in "${!RUN_LABELS[@]}"; do
    ...
    echo "====================================================="
    echo "Running conditional Vendi evaluation with timestep-stage0 residual"
    ...
    echo "  residual_origin_layer : ${RESIDUAL_ORIGIN_LAYER}"
    echo "  residual_target_layers: ${RESIDUAL_TARGET_LAYERS}"
    echo "  residual_weights      : ${RESIDUAL_WEIGHTS}"
    echo "  residual_use_layernorm: ${RESIDUAL_USE_LAYERNORM}"
    echo "  timestep_stage        : ${TIMESTEP_STAGE}"
