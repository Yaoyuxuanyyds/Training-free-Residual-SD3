...
if __name__ == '__main__':
    ...
    if args.overwrite:
        ...
        #     results = json.load(file)
        # 优先查找你的自定义文件：sample_results.json
        custom_results_file = os.path.join(args.load_dir, "sample_results.json")
        # 备用文件：原来的 results-{args.load_name}.json
        default_results_file = os.path.join(args.load_dir, f"results-{args.load_name}.json")
        
        if os.path.exists(custom_results_file):
            # 找到自定义文件，读取
            with open(custom_results_file, "r", encoding="utf-8") as file:
                results = json.load(file)
            print(f"✅ 成功读取自定义结果文件：{custom_results_file}")
        elif os.path.exists(default_results_file):
            # 找不到自定义文件，读取默认文件
            with open(default_results_file, "r", encoding="utf-8") as file:
                results = json.load(file)
            print(f"✅ 成功读取默认结果文件：{default_results_file}")
        else:
            # 两个文件都找不到，报错并提示
            raise FileNotFoundError(
                f"未找到结果文件！\n"
                f"已尝试查找以下两个文件：\n"
                f"1. 你的自定义文件：{custom_results_file}\n"
                f"2. 默认文件：{default_results_file}\n"
                f"请确保其中一个文件存在于目录中。"
            )
    ...
    for benchmark_type in benchmark_types:
        ...
        if benchmark_type == "ImageReward-v1.0":
            # model = RM.load(name="/inspire/hdd/project/chineseculture/public/yuxuan/base_models/Eval/ImageReward-v1.0", device="cuda")
            model = RM.load(
                name="/inspire/hdd/project/chineseculture/public/yuxuan/base_models/Eval/ImageReward/ImageReward.pt",
                device="cuda",
                med_config="/inspire/hdd/project/chineseculture/public/yuxuan/base_models/Eval/ImageReward/med_config.json"
            )
            ...
            model = RM.load_score(
                name=benchmark_type, device="cuda", download_root="/inspire/hdd/project/chineseculture/public/yuxuan/base_models/Eval/ImageReward"
            )
        ...
        with torch.no_grad():
            ...
            for i in pbar:
                ...
                if not os.path.isfile(img_path):
                    print(f"❌ 严重错误：图片文件不存在！拼接后的路径：{img_path}")
                    print(f"  - args.load_dir: {args.load_dir}")
                    print(f"  - args.load_name: {args.load_name}")
                    print(f"  - results[i]['img_path']: {results[i]['img_path']}")
                    exit(1)  # 直接退出，先修复路径问题
