...
CUDA_VISIBLE_DEVICES=0 python compute_flux_text_procrustes.py \
  ...
  --num-samples 5000 \
  --origin-layer 0 \
  --target-layer-start 1 \
  --residual_use_layernorm $RES_USE_LAYERNORM \
  --output /inspire/hdd/project/chineseculture/public/yuxuan/Training-free-Residual-SD3/Flux-Residual/logs/procrustes_rotations/procrustes_rotations_coco5k_ln_t1-full-o0.pt \
  --col-center \
  --timestep-buckets 1 \
