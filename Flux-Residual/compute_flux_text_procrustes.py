...
def run(args: argparse.Namespace):
    ...
    origin_chunks = [[] for _ in range(num_buckets)]
    target_chunks = {layer: [[] for _ in range(num_buckets)] for layer in target_layers}
    ...
    for bucket_idx in range(num_buckets):
        ...
        X_final = X_ln - X_ln.mean(dim=0, keepdim=True) if args.col_center else X_ln
        num_valid_tokens.append(X_final.shape[0])

        rotations: List[torch.Tensor] = []
        for layer in target_layers:
            Y_ln = apply_simulated_ln(target_chunks[layer][bucket_idx])
            Y_final = Y_ln - Y_ln.mean(dim=0, keepdim=True) if args.col_center else Y_ln
