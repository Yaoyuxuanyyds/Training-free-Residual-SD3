#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Precompute orthogonal Procrustes rotations for Flux text residual alignment."""
...
def encode_image_to_latent(pipe: FluxPipelineWithRES, img_tensor: torch.Tensor) -> torch.Tensor:
    vae = pipe.vae
...
def run(args: argparse.Namespace):
    ...
    num_buckets = max(int(args.timestep_buckets), 1)
    ...
    layer_counts = {layer: [0 for _ in range(num_buckets)] for layer in target_layers}
    layer_sums = {layer: [None for _ in range(num_buckets)] for layer in target_layers}
    layer_sumsq = {layer: [None for _ in range(num_buckets)] for layer in target_layers}
    layer_cross = {layer: [None for _ in range(num_buckets)] for layer in target_layers}

    for pair_idx, image_source, prompt_value in tqdm(
        _iterate_pairs(args, dataset),
        desc="[Pairs]",
    ):
        prompt = _normalize_prompt(prompt_value)
        gt_pil = load_and_resize_pil(image_source, args.height, args.width)
        ...
        if len(prompt_outputs) == 4:
            ...
            prompt_emb, pooled_emb, text_ids = prompt_outputs
            token_mask = None
