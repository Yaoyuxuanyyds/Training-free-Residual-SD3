...
CUDA_VISIBLE_DEVICES=0 python compute_flux_text_procrustes.py \
  ...
  --num-samples 5000 \
  --origin-layer 4 \
  --target-layer-start 5 \
  --output /inspire/hdd/project/chineseculture/public/yuxuan/Training-free-Residual-SD3/Flux-Residual/logs/procrustes_rotations/procrustes_rotations_coco5k_ln_t1-full-o4.pt \
  --col-center \
  --timestep-buckets 1 \
