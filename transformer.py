...
class SD3Transformer2DModel_Residual(nn.Module):
    ...
    def forward(
        ...
        force_text_dependency: bool = False,
        force_text_dependency_scale: float = 1.0,
        ...
        if force_text_dependency:
            if encoder_hidden_states.shape[-1] != hidden_states.shape[-1]:
                raise ValueError(
                    "force_text_dependency expects encoder_hidden_states dim to match hidden_states dim "
                    f"(got {encoder_hidden_states.shape[-1]} vs {hidden_states.shape[-1]})."
                )
            text_summary = encoder_hidden_states.mean(dim=1, keepdim=True)
            hidden_states = hidden_states + force_text_dependency_scale * text_summary
