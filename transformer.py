...
def _joint_transformer_block_forward_a2_pre(
    ...
    if self.context_pre_only:
        ...
        if use_inject:
            ...
            phi_eps = residual_inject_cfg.get("phi_eps", 1e-6)
            phi_unbiased = residual_inject_cfg.get("phi_unbiased", False)
        ...
        if use_inject:
            ...
            x_phi_source = x_res.detach() if stop_grad else x_res
            o_phi_source = origin_context_states.detach() if stop_grad else origin_context_states
            x_phi = _tokenwise_phi(x_phi_source, eps=phi_eps, unbiased=phi_unbiased)
            o_phi = _tokenwise_phi(o_phi_source, eps=phi_eps, unbiased=phi_unbiased)
            if rotation_matrix is not None:
                o_phi = torch.matmul(o_phi, rotation_matrix)
            delta = o_phi * weight
